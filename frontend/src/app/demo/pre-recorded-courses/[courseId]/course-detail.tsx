...
function ModuleBlock({
  ...
  onToggle: () => void;
  onSelect: (item: Item, thumb: number) => void;
}) {
  const doneCount = module.items.filter((i) => completedIds.has(i.id)).length;
  return (
    <div className="overflow-hidden rounded-lg border border-border">
      <button
        ...
        onClick={onToggle}
        aria-expanded={open}
        className="flex w-full items-center gap-3 bg-ink-50/60 px-3 py-2.5 text-left transition hover:bg-ink-100/60"
      >
        ...
        <div className="min-w-0 flex-1">
          <p className="truncate text-sm font-medium text-ink-900">{module.title}</p>
      ...
      {open ? (
        <ul className="divide-y divide-border">
          {module.items.map((item, i) => {
            const { Icon, className } = TYPE_META[item.type];
            ...
            return (
              <li key={item.id}>
                <button
                  type="button"
                  onClick={() => onSelect(item, index * 3 + i)}
                ...
                >
                  ...
                  />
                  <div className="min-w-0 flex-1">
                    <p className={"truncate text-sm " + (active ? "font-semibold text-ink-900" : "font-medium text-ink-800")}>
                      {item.title}
                    </p>
                    <p className="text-xs text-muted-foreground">{metaTail(item)}</p>
                  </div>
                  {done ? (
                    <CheckCircle size={16} weight="fill" className="shrink-0 text-saffron-700" />
                  ) : active ? (
                    <PlayCircle size={18} weight="fill" className="shrink-0 text-brand-600" />
                  ) : null}
                </button>
              </li>
            );
...
/* ----------------------------- Lesson stage (inline, main pane) ----------------------------- */
function LessonStage({ item, thumb }: { item: Item; thumb: number }) {
  const { label } = TYPE_META[item.type];
  const quiz = item.quiz;

  // Quiz run state — this component is remounted per lesson via `key`, so it
  // resets on lesson change without an effect.
  const [qIndex, setQIndex] = useState(0);
  const [answers, setAnswers] = useState<(number | null)[]>(
    quiz ? Array(quiz.length).fill(null) : [],
  );
  const [revealed, setRevealed] = useState<boolean[]>(
    quiz ? Array(quiz.length).fill(false) : [],
  );
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-border bg-surface">
      {/* Video lessons get the theater stage in the main pane */}
      {item.type === "video" ? (
        <div className="relative w-full" style={{ background: THUMBS[thumb % THUMBS.length] }}>
      ...
      <div className="px-5 py-4 sm:px-6 sm:py-5">
        ...
        <h2 className="mt-2 font-display text-xl text-ink-900 sm:text-2xl">{item.title}</h2>

        {item.body ? (
          <p className="mt-3 max-w-2xl text-sm leading-relaxed text-ink-700">{item.body}</p>
        ) : null}

        {/* Quiz — multi-question, inline */}
        {item.type === "quiz" && quiz ? (
          finished ? (
        ...
        {/* Assignment — upload form (demo, no real submit) */}
...
export function CourseDetail({ course }: { course: Course }) {
  // Flatten every lesson into a single playlist order (module by module).
  const playlist = useMemo<Selected[]>(
    () =>
      course.modules.flatMap((m, mi) =>
        m.items.map((item, ii) => ({ item, thumb: mi * 3 + ii, moduleId: m.id })),
      ),
    [course],
  );
  ...
  const [completedIds, setCompletedIds] = useState<Set<string>>(
    () => new Set(course.modules.flatMap((m) => m.items.filter((i) => i.done).map((i) => i.id))),
  );

  const current = playlist[index] ?? null;

  function goTo(nextIndex: number) {
    const clamped = Math.max(0, Math.min(playlist.length - 1, nextIndex));
    setIndex(clamped);
    setOpenModule(playlist[clamped].moduleId);
  }

  function selectItem(item: Item) {
    const i = playlist.findIndex((p: Selected) => p.item.id === item.id);
    if (i >= 0) goTo(i);
  }
  ...
  function completeAndContinue(id: string) {
    setCompletedIds((prev) => new Set(prev).add(id));
    if (index < playlist.length - 1) goTo(index + 1);
  }

  const completedCount = completedIds.size;
  const totalLessons = playlist.length;
    ...
    <div className="space-y-5">
      ...
      <div className="rounded-xl border border-border bg-surface p-5">
        ...
        <div className="mt-4 flex items-center gap-3">
          <Progress value={pct} className="flex-1" label={`${course.title} progress`} />
      ...
      {/* Main pane (lesson stage) + curriculum */}
      <div className="grid gap-6 lg:grid-cols-[1fr_20rem]">
        <div className="space-y-5">
          {current ? (
            <>
              <LessonStage key={current.item.id} item={current.item} thumb={current.thumb} />

              {/* Playlist navigation — move through lessons like a Udemy course */}
              <div className="flex items-center justify-between gap-3 rounded-xl border border-border bg-surface p-3">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <Button
                    variant={completedIds.has(current.item.id) ? "outline" : "ghost"}
                    size="sm"
                    onClick={() => toggleComplete(current.item.id)}
                  >
                    <CheckCircle
                      size={15}
                      weight={completedIds.has(current.item.id) ? "fill" : "regular"}
                      className={completedIds.has(current.item.id) ? "text-saffron-700" : ""}
                    />
                    {completedIds.has(current.item.id) ? "Completed" : "Mark complete"}
                  </Button>
                </div>

                {index < totalLessons - 1 ? (
                  <Button size="sm" onClick={() => completeAndContinue(current.item.id)}>
                    Next lesson <CaretRight size={15} />
                  </Button>
                ) : (
                  <Button
                    size="sm"
                    onClick={() => toggleComplete(current.item.id)}
                    disabled={completedIds.has(current.item.id)}
                  >
                    Finish course
                  </Button>
                )}
        ...
        {/* Curriculum — collapsible, drives the main pane */}
